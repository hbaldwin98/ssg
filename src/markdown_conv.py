...
def split_nodes_links(old_nodes: List[TextNode]) -> List[HTMLNode]:
...
def split_nodes_images(old_nodes: List[TextNode]) -> List[HTMLNode]:
    def split_recurse(text: str) -> List[str]:
        if text == "":
            return []
...
def markdown_to_nodes(text: str) -> List[TextNode]:
    ...
    nodes.extend(split_nodes_delimiter(nodes, "**", TextType.Bold))
    nodes.extend(split_nodes_delimiter(nodes, "*", TextType.Italic))
    nodes.extend(split_nodes_delimiter(nodes, "`", TextType.Code))
    nodes.extend(split_nodes_images(nodes))
    nodes.extend(split_nodes_links(nodes))
